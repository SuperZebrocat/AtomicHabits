...
def validate_periodicity(periodicity):
    ...
    if not 0 < periodicity <= 7:
        raise ValidationError(f'Привычку нельзя выполнять реже одного раза в неделю')


def validate_lead_time(lead_time):
    """Валидация времени выполнения привычки"""
    if lead_time > 120:
        raise ValidationError(f'Время выполнения привычки не должно превышать 2-х минут')
